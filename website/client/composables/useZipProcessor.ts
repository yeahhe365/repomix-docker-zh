...
interface ZipProcessorMessages {
  failedToCreateZip?: (message: string) => string;
  uploadZipFile?: string;
  unknownError?: string;
}

export function useZipProcessor(messages: ZipProcessorMessages = {}) {
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        zip(fileMap, (err, data) => {
          if (err) {
            reject(new Error(messages.failedToCreateZip?.(err.message) ?? `Failed to create ZIP file: ${err.message}`));
          } else {
  ...
  function validateZipFile(file: File): { valid: boolean; error?: string } {
    if (file.type !== 'application/zip' && !file.name.endsWith('.zip')) {
      return { valid: false, error: messages.uploadZipFile ?? 'Please upload a ZIP file' };
    }
    return { valid: true };
  }
