...
export function useFileUpload(config: FileUploadConfig) {
  ...
  function defaultValidateFiles(files: File[]): { valid: boolean; error?: string } {
    if (files.length === 0) {
      return { valid: false, error: config.messages?.noFilesFound ?? 'No files found' };
    }
    ...
    if (totalSize > maxFileSize) {
      ...
      return {
        valid: false,
        error:
          config.messages?.totalSizeTooLarge?.(sizeMB, limitMB) ??
          `Total size (${sizeMB}MB) exceeds the ${limitMB}MB limit`,
      };
    }
